...
const PaymentPage = () => {
  ...
  // Destructure asset and amount from location.state to display transaction details
  const { type, asset, amount } = location.state || {};
  const [error, setError] = useState('');

  // Hardcoded bank details as required
  const bankDetails = {
    bankName: "FNB BANK",
  ...
  const handleConfirmPayment = () => {
    // For demonstration, simulate a failed payment confirmation
