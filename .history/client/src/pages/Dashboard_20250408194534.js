...
const Dashboard = () => {
  ...
  // Calculate total balance based on portfolio assets and prices
          ...
          Object.entries(userData.portfolio).map(([asset, amount]) => (
            <div key={asset} className="asset-item">
              ...
              <span className="asset-amount">
                {parseFloat(amount).toFixed(4)} (≈ R{' '}
                {userData.assetPrices && userData.assetPrices[asset]
                  ? (parseFloat(amount) * parseFloat(userData.assetPrices[asset])).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })
                  : '0.00'}
                )
              </span>
            </div>
